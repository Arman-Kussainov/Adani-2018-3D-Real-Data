...
// проецируемых пикселей ci и ck рентгеновских проекций
		...
		if ((mammo==0)&&(a_ffix == "XY")) {
			...
			for (int Zt = t_arget - d_elta; Zt <= t_arget + d_elta; Zt++) {
				float Zp = static_cast<float>(Zt - half_slice)+85.0f;
// распараллеливание процесса обратного проецирования
#pragma omp parallel for
				for (int Xt = 0; Xt < slice_size; Xt++) {
					float Xt_cos = static_cast<float>(Xt-25 - half_slice)*c_os;
					float Xt_sin = static_cast<float>(Xt-25 - half_slice)*s_in;
					for (int Yt = 0; Yt < slice_size; Yt++) {
						float Y_t = static_cast<float>(Yt - half_slice-5);

						float Xp = Xt_cos - Y_t*s_in-178.0f;
...
#pragma omp parallel for
				for (int Xt = 0; Xt < slice_size; Xt++) {
					...
					float Xp = X_t*c_os - Yt_sin - 178.0f;
...
// создание и применение маски М для удаления фона изображения
	double min, max;
	...
	//double b_rightness=20000.0, c_ontrast=6.3;
	//slice_all = slice_all * c_ontrast;
	//slice_all = slice_all + b_rightness;
	//slice_all.setTo(max, slice_all > min);

	cv::normalize(slice_all, slice_normalized, 0, 65535, CV_MINMAX, CV_16UC1);
	//cv::normalize(M_ask, slice_normalized, 0, 65535, CV_MINMAX, CV_16UC1);

	//	обработка исходного изображения и запись на диск
	string n_ame = "_slices_";
	string pat_h = "C:\\Users\\Martha\\Desktop\\slices\\paper_data\\";
	//string pat_h = "";
	...
	string filenam_e = pat_h + c_ounter + "_" + SSTR(2*d_elta+1) + "_" + a_ffix + n_ame + rotation_s + "_" + slic_e + "_" + time_stamp + extensio_n;
	
//	Mat cut_image = slice_normalized(Rect(600, 50, 800, 1300));
