...
if (3.0f < 2.0f) {
	int Tp_x = 943, Tp_y = 600, Tp_width = 1800, Tp_height = 1800;

	Mat sum_image = cv::Mat(Tp_height, Tp_width, CV_32FC1, Scalar::all(0));
	Mat norm_image = cv::Mat(Tp_height, Tp_width, CV_16UC1, Scalar::all(0));
	Mat image_float = cv::Mat(Tp_height, Tp_width, CV_32FC1, Scalar::all(0));
	Mat	first_image, target_roi, current_roi, test_im;

	Mat zero_image_float = cv::Mat(Tp_height, Tp_width, CV_32FC1, Scalar::all(0));
	string another = "C:\\adani_data_correction\\sample_1\\RetrvdImg_0 гр.tif";
	Mat zero_image = cv::imread(another, CV_LOAD_IMAGE_UNCHANGED);

	double a_lpha = 15.0;
	double b_eta = 20000;

	Mat zero_new_image;// = Mat::zeros(image.size(), CV_32FC1);
	zero_image.convertTo(zero_new_image, -1, a_lpha, b_eta);

	Mat zero_cut_image;
	zero_cut_image = zero_new_image(Rect(Tp_x, Tp_y, Tp_width, Tp_height));
	zero_cut_image.convertTo(zero_image_float, CV_32FC1);

	string nam_e = "new_image_";
	string inde_x = SSTR(0);
	string local_path = "C:\\adani_data_correction\\clean_data\\";
	string extensio_n = ".png";
	string full_filename = local_path + nam_e + inde_x + extensio_n;


	bitwise_not(zero_new_image, zero_new_image);
	cv::normalize(zero_new_image, zero_new_image, 0, 65535, CV_MINMAX, CV_16UC1);

	vector<int> compression_params;
	compression_params.push_back(CV_IMWRITE_PNG_COMPRESSION);
	compression_params.push_back(9);
	cv::imwrite(full_filename, zero_new_image, compression_params);


	Mat Mask_0, M_ask;
	inRange(zero_cut_image, 0, 50900, Mask_0);
	Mask_0.convertTo(Mask_0, CV_32FC1, 1.f / 255);
	zero_image_float = zero_image_float.mul(Mask_0);
	first_image = zero_image_float;
	target_roi = Mask_0(Rect(450, 450, 900, 900));
	//*********

	namedWindow("raw_data", WINDOW_NORMAL);

	for (int even_counter = 2; even_counter <= 230; even_counter += 2) {

		string c_ounter = SSTR(even_counter);
		string path_name = "C:\\adani_data_correction\\sample_1\\RetrvdImg_";
		string rest_of_the_name = " гр.tif";
		string filenam_e = path_name + c_ounter + rest_of_the_name;

		Mat image = cv::imread(filenam_e, CV_LOAD_IMAGE_UNCHANGED);
		Mat new_image;// = Mat::zeros(image.size(), CV_32FC1);
		Mat copy_new_image = Mat::zeros(image.size(), CV_16UC1);
		image.convertTo(new_image, -1, a_lpha, b_eta);

		int re_scale = 4;
		resizeWindow("raw_data", image.cols / re_scale, image.rows / re_scale);

		Mat cut_image;
		cut_image = new_image(Rect(Tp_x, Tp_y, Tp_width, Tp_height));
		cut_image.convertTo(image_float, CV_32FC1);

		inRange(cut_image, 0, 50900, M_ask);
		M_ask.convertTo(M_ask, CV_32FC1, 1.f / 255);
		image_float = image_float.mul(M_ask);

		int image_shift = 0;
		current_roi = M_ask(Rect(450 + image_shift, 450, 900, 900));
		float imag_diff = sum(abs(current_roi - target_roi))[0];
		float plato_counter = 0;

		float s_ign = 1.0f, s_tep = 1.0f;
		int numbers_of_change = 1;
		while (numbers_of_change <=2 ) {
			
			image_shift += s_ign*s_tep;
			
			current_roi = M_ask(Rect(450 + image_shift, 450, 900, 900));
			float current_diff = sum(abs(current_roi - target_roi))[0];

			if (current_diff > imag_diff) { s_ign *= -1.0f; numbers_of_change++; }
			else { imag_diff = current_diff; }
			
			//imshow("raw_data", current_roi - target_roi);
			//cout << image_shift << "*"<< current_diff<<"\t";
			//waitKey(5);
		}

		image_shift -= (-s_ign*s_tep);

		//image_shift = int(round(float(image_shift)/2.0f));
	
		current_roi = M_ask(Rect(450 + image_shift, 450, 900, 900));
		float current_diff = sum(abs(current_roi - target_roi))[0];
		//cout << even_counter<<"*"<<image_shift << "*" << current_diff << "<\n";
		//imshow("raw_data", current_roi - target_roi);


		cut_image = new_image(Rect(Tp_x + image_shift, Tp_y, Tp_width, Tp_height));
		//cut_image.copyTo(copy_new_image(cv::Rect(Tp_x, Tp_y, Tp_width, Tp_height)));

		string nam_e = "new_image_";
		string inde_x = SSTR(even_counter);
		string local_path = "C:\\adani_data_correction\\clean_data\\";
		string extensio_n = ".png";
		string full_filename = local_path + nam_e + inde_x + extensio_n;

		bitwise_not(new_image, new_image);
		cv::normalize(new_image, new_image, 0, 65535, CV_MINMAX, CV_16UC1);

		vector<int> compression_params;
		compression_params.push_back(CV_IMWRITE_PNG_COMPRESSION);
		compression_params.push_back(9);
		cv::imwrite(full_filename, new_image, compression_params);
	...
	// if (3.0f < 2.0f) {
		...
		if (populate_projections == 1) {
			...
			double main_time1 = omp_get_wtime() - main_time;
	...
	//}
		std::cout << "done!\n";
		double main_time2 = omp_get_wtime() - main_time;
		std::printf("\ntime to complete calculations in seconds = %f\n", main_time2);
		getchar();
	}
